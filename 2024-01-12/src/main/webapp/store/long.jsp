             ...
             <div class="col-12">
			     <div class="pagination d-flex justify-content-center mt-5">
                 	<c:if test="${startPage>1 }">
                    	<a href="long.do?page=${startPage-1}" class="rounded">&laquo;</a>
                    </c:if>
                    <c:forEach var="i" begin="${startPage }" end="${endPage }">
                    	<a href="long.do?page=${i}" class="${curpage==i?"active rounded":"rounded" }">${i }</a>	
                    </c:forEach>
                    <c:if test="${endPage<totalpage }">
                    	<a href="long.do?page=${endPage+1}" class="rounded">&raquo;</a>
                    </c:if>
                  </div>
              </div>
    	</div>     
